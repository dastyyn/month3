...
async def answer_age(message: types.Message, state: FSMContext):
    ...
    if not age.isdigit():
        ...
        kb = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(text="Ежедневно"),
                    types.InlineKeyboardButton(text="Часто")
                ],
                [
                    types.InlineKeyboardButton(text="Редко"),
                    types.InlineKeyboardButton(text="Не смотрю")
                ]
            ]
        )
        await state.set_state(Questions.view_count)
        await message.answer("Продолжаем. Как часто вы смотрите аниме?",
                             reply_markup=kb)
